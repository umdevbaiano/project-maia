...
import { STATUS_LABELS, PRIORIDADE_LABELS, PRIORIDADE_COLORS, STATUS_COLORS } from '../types/prazo';
        ...
        <div className="p-8">
            <div className="flex items-center justify-between mb-8">
                <div>
                    ...
                    <p className="text-zinc-400 text-sm mt-1">{prazos.length} prazo(s)</p>
                </div>
            ...
            <div className="flex gap-2 mb-6">
                {['', 'pendente', 'cumprido', 'expirado'].map(s => (
                    <button key={s} onClick={() => setFilter(s)}
                        className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${filter === s ? 'bg-blue-600 text-white' : 'bg-zinc-900 text-zinc-400 hover:bg-zinc-800'
                            }`}>
            ...
            {loading ? (
                ...
                <div className="text-center py-20">
                    ...
                    <p className="text-zinc-500">Nenhum prazo encontrado.</p>
                </div>
            ) : (
                <div className="space-y-3">
                    {prazos.map(prazo => {
                        ...
                        return (
                            <div key={prazo.id}
                                className={`bg-zinc-900 border rounded-xl p-5 flex items-center justify-between transition-all group ${overdue ? 'border-red-500/30 bg-red-500/5' : urgent ? 'border-orange-500/20' : 'border-zinc-800 hover:border-zinc-700'
                                    }`}>
                                <div className="flex items-center gap-4">
                                    <button onClick={() => toggleStatus(prazo)}
                                        className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all ${prazo.status === 'cumprido' ? 'border-green-500 bg-green-500' : 'border-zinc-600 hover:border-blue-500'
                                            }`}>
                                        {prazo.status === 'cumprido' && <CheckCircle2 className="w-4 h-4 text-white" />}
                                    </button>
                                    <div>
                                        <h3 className={`font-medium text-sm ${prazo.status === 'cumprido' ? 'text-zinc-500 line-through' : 'text-white'}`}>
                                            {prazo.titulo}
                                        </h3>
                                        <div className="flex items-center gap-3 mt-1 flex-wrap">
                                            ...
                                            <span className={`text-xs ${overdue ? 'text-red-400 font-medium' : urgent ? 'text-orange-400' : 'text-zinc-500'}`}>
                                                📅 {new Date(prazo.data_limite).toLocaleDateString('pt-BR')}
                                                {prazo.status === 'pendente' && (
                                                    overdue ? ` (${Math.abs(days)} dia(s) atrás)` : ` (${days} dia(s))`
                                                )}
                                            </span>
                                            {prazo.caso_titulo && <span className="text-xs text-zinc-600">📁 {prazo.caso_titulo}</span>}
                                        </div>
                                    </div>
                                </div>
                                <div className="opacity-0 group-hover:opacity-100 transition-opacity flex gap-1">
                                    <button onClick={() => openEdit(prazo)} className="text-zinc-400 hover:text-blue-400 p-1"><Edit3 className="w-4 h-4" /></button>
                                    <button onClick={() => handleDelete(prazo.id)} className="text-zinc-400 hover:text-red-400 p-1"><Trash2 className="w-4 h-4" /></button>
                                </div>
                            </div>
                        );
                    })}
                ...
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-zinc-900 border border-zinc-800 rounded-2xl w-full max-w-lg p-6 shadow-2xl">
                        <div className="flex items-center justify-between mb-6">
                            <h2 className="text-lg font-semibold text-white">{editing ? 'Editar Prazo' : 'Novo Prazo'}</h2>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div><label className="block text-sm text-zinc-300 mb-1">Data Limite *</label>
                                    <input type="date" value={form.data_limite} onChange={e => setForm({ ...form, data_limite: e.target.value })}
                                        className="w-full bg-zinc-800/50 border border-zinc-700 rounded-lg px-3 py-2.5 text-sm text-zinc-100 focus:outline-none focus:ring-2 focus:ring-blue-600" /></div>
                                <div><label className="block text-sm text-zinc-300 mb-1">Prioridade</label>
