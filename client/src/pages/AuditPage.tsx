            ...
            <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-zinc-400 text-sm">Registro de todas as ações na plataforma</p>
                    </div>
                </div>
                <span className="text-sm text-zinc-500">{total} registros</span>
            </div>
            ...
            <div className="flex gap-3 mb-6">
                <div className="relative">
                    ...
                    <select
                        ...
                        className="pl-9 pr-4 py-2 bg-zinc-800 border border-zinc-700 rounded-lg text-white text-sm appearance-none cursor-pointer min-w-[160px] focus:outline-none focus:border-blue-500"
                    >
            ...
            {loading ? (
                ...
                <div className="space-y-1">
                    {logs.map((log) => {
                        ...
                        const colorClass = ACTION_COLORS[log.action] || 'text-zinc-400 bg-zinc-400/10';
                        const resourceLabel = RESOURCE_LABELS[log.resource_type] || log.resource_type;

                        return (
                            <div
                                key={log.id}
                                className="flex items-center gap-4 px-4 py-3 bg-zinc-900 border border-zinc-800 rounded-lg hover:border-zinc-700 transition-colors"
                            >
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-xs text-zinc-500">{resourceLabel}</span>
                                        {log.details && (
                                            <span className="text-xs text-zinc-400 truncate">— {log.details}</span>
                                        )}
                                    </div>
                                    <div className="flex items-center gap-2 mt-1">
                                        <span className="text-xs text-zinc-500">{log.user_email || log.user_id}</span>
                                        {log.resource_id && (
                                            <span className="text-[10px] text-zinc-600 font-mono">ID: {log.resource_id.slice(0, 8)}…</span>
                                        )}
                                    </div>
                                </div>
                                <span className="text-xs text-zinc-500 whitespace-nowrap">{formatDate(log.timestamp)}</span>
                            </div>
                        );
                    })}
                </div>
            ...
            {totalPages > 1 && (
                <div className="flex items-center justify-center gap-4 mt-6">
                    <button
                        ...
                        disabled={page <= 1}
                        className="p-2 rounded-lg bg-zinc-800 text-zinc-400 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed"
                    >
