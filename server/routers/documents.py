...
async def upload_document(background_tasks: BackgroundTasks, file: UploadFile = File(...), cliente_id: Optional[str] = Form(None), caso_id: Optional[str] = Form(None), current_user: dict = Depends(get_current_user_or_demo)):
...
@router.get("/")
async def list_documents(current_user: dict = Depends(get_current_user_or_demo)):
    try:
...
async def link_document(doc_id: str, request: LinkDocumentRequest, current_user: dict = Depends(get_current_user_or_demo)):
    try:
        db = get_database()
...
async def delete_document(doc_id: str, current_user: dict = Depends(get_current_user_or_demo)):
    try:
        db = get_database()
