...
async def create_prazo(request: PrazoCreateRequest, current_user: dict = Depends(get_current_user)):
    db = get_database()
    result = await prazo_service.create_prazo(
    ...
    await audit_service.log_action(
        ...
        user_email=current_user.get("email", ""), action="CREATE", resource_type="prazo",
        resource_id=result.get("id", ""), details=f"Prazo: {request.titulo}",
    )
    return result
...
async def update_prazo(prazo_id: str, request: PrazoUpdateRequest, current_user: dict = Depends(get_current_user)):
    ...
    await audit_service.log_action(
        db, workspace_id=current_user["_workspace_id"], user_id=current_user["_user_id"],
        user_email=current_user.get("email", ""), action="UPDATE", resource_type="prazo",
        resource_id=prazo_id,
    )
    return prazo
...
async def delete_prazo(prazo_id: str, current_user: dict = Depends(get_current_user)):
    ...
    await audit_service.log_action(
        db, workspace_id=current_user["_workspace_id"], user_id=current_user["_user_id"],
        user_email=current_user.get("email", ""), action="DELETE", resource_type="prazo",
        resource_id=prazo_id,
    )
