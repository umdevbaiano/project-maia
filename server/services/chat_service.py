from datetime import datetime, timezone
import logging
import re
from typing import Optional, AsyncGenerator
from bson import ObjectId
from motor.motor_asyncio import AsyncIOMotorDatabase
from core.ai.base import AIProvider
from core.ai.orchestrator import MaiaOrchestrator
from core.rag import pipeline as rag
from core.security.encryption import encrypt_field, decrypt_field
from models.peca import TipoPeca
from services import peca_service

logger = logging.getLogger(__name__)

COLLECTION_NAME = "chat_history"

def _serialize_message(doc: dict) -> dict:
    if not doc:
        return {}
    
    doc["id"] = str(doc.pop("_id"))
    if isinstance(doc.get("timestamp"), datetime):
        doc["timestamp"] = doc["timestamp"].isoformat()
    
    if doc.get("content"):
        doc["content"] = decrypt_field(doc["content"])
        
    doc.pop("workspace_id", None)
    doc.pop("user_id", None)
    return doc

async def _process_maia_save_tag(
    reply: str,
    db: AsyncIOMotorDatabase,
    workspace_id: str,
    user_id: str,
    caso_id: Optional[str] = None,
) -> tuple[str, Optional[str]]:
    match = re.search(r'<!--\s*MAIA_SAVE:\s*(.*?)\s*-->', reply)
    if not match:
        return reply, None

    tag_value = match.group(1).strip()
    try:
        tipo = TipoPeca(tag_value)
        payload_match = re.search(r'\[INICIO_PECA\](.*?)\[FIM_PECA\]', reply, re.DOTALL)
        clean_payload = (
            payload_match.group(1).strip() if payload_match
            else re.sub(r'<!--\s*MAIA_SAVE:\s*.*?\s*-->', '', reply).strip()
        )
        
        await peca_service.save_peca_direct(
            db, workspace_id, user_id, tipo,
            titulo=tipo.value.replace('_', ' ').title(),
            conteudo=clean_payload, caso_id=caso_id
        )
        
        success_msg = "\n\n\u2705 *Peça salva automaticamente no sistema em **Peças Jurídicas**.*"
        clean_reply = reply.replace('[INICIO_PECA]', '').replace('[FIM_PECA]', '')
        clean_reply = re.sub(r'<!--\s*MAIA_SAVE:\s*.*?\s*-->', '', clean_reply).strip()
        
        return clean_reply + success_msg, success_msg
    except ValueError:
        return reply, None

async def get_chat_history(
    db: AsyncIOMotorDatabase,
    workspace_id: str,
    caso_id: Optional[str] = None,
) -> list[dict]:
    query = {"workspace_id": workspace_id, "caso_id": caso_id} if caso_id else {"workspace_id": workspace_id, "caso_id": {"$exists": False}}
    cursor = db[COLLECTION_NAME].find(query).sort("timestamp", 1)
    return [_serialize_message(doc) async for doc in cursor]

async def _get_case_context(db: AsyncIOMotorDatabase, caso_id: str, workspace_id: str) -> str:
    try:
        caso = await db["casos"].find_one({"_id": ObjectId(caso_id), "workspace_id": workspace_id})
        if not caso:
            return ""

        context = [
            "📋 CONTEXTO DO PROCESSO (chat vinculado):",
            f"- Título: {caso.get('titulo', 'N/A')}",
            f"- Número: {caso.get('numero', 'N/A')}",
            f"- Tipo: {caso.get('tipo', 'N/A')}",
            f"- Status: {caso.get('status', 'N/A')}",
            f"- Descrição: {caso.get('descricao', 'N/A')}"
        ]

        if caso.get("cliente_id"):
            cliente = await db["clientes"].find_one({"_id": ObjectId(caso["cliente_id"])})
            if cliente:
                context.append(f"- Cliente: {cliente.get('nome', 'N/A')}")
                context.append(f"- Documento: {cliente.get('documento', 'N/A')}")

        prazos = db["prazos"].find({"caso_id": caso_id, "workspace_id": workspace_id}).sort("data_limite", 1).limit(5)
        prazos_list = [f"  • {p.get('titulo', 'N/A')} — {p.get('data_limite', 'N/A')} ({p.get('status', 'N/A')})" async for p in prazos]
        
        if prazos_list:
            context.append("- Prazos vinculados:")
            context.extend(prazos_list)

        return "\n".join(context) + "\n\nResponda SEMPRE considerando os dados deste processo. Se o advogado perguntar algo genérico, relacione ao caso quando possível.\n"

    except Exception as e:
        logger.warning("Error building case context: %s", e)
        return ""

async def _prepare_chat_context(
    db: AsyncIOMotorDatabase,
    message: str,
    workspace_id: str,
    user_id: str,
    caso_id: Optional[str] = None,
):
    collection = db[COLLECTION_NAME]

    user_message = {
        "role": "user",
        "content": encrypt_field(message),
        "timestamp": datetime.now(timezone.utc),
        "workspace_id": workspace_id,
        "user_id": user_id,
    }
    if caso_id:
        user_message["caso_id"] = caso_id
    await collection.insert_one(user_message)

    history_query = {"workspace_id": workspace_id, "caso_id": caso_id} if caso_id else {"workspace_id": workspace_id, "caso_id": {"$exists": False}}
    cursor = collection.find(history_query).sort("timestamp", -1).limit(10)
    
    context = []
    async for doc in cursor:
        content = decrypt_field(doc["content"]) if doc.get("content") else ""
        context.append({"role": doc["role"], "content": content})
    context.reverse()

    case_context_str = await _get_case_context(db, caso_id, workspace_id) if caso_id else ""

    rag_chunks, legal_chunks = [], []
    try:
        search_query = f"{case_context_str[:200]} {message}" if case_context_str else message
        result = await rag.retrieve_hybrid(workspace_id, search_query)
        rag_chunks = result.get("workspace", [])
        legal_chunks = result.get("legal", [])
    except Exception as e:
        logger.warning("RAG retrieval failed: %s", e)

    full_prompt = f"{case_context_str}\n{message}" if case_context_str else message
    return collection, context, full_prompt, rag_chunks, legal_chunks

async def send_message(
    db: AsyncIOMotorDatabase,
    message: str,
    ai_provider: AIProvider,
    workspace_id: str,
    user_id: str,
    caso_id: Optional[str] = None,
) -> str:
    collection, context, full_prompt, rag_chunks, _ = await _prepare_chat_context(
        db, message, workspace_id, user_id, caso_id
    )

    orchestrator = MaiaOrchestrator(ai_provider)
    ai_reply = await orchestrator.chat(full_prompt, context, rag_context=rag_chunks)
    ai_reply, _ = await _process_maia_save_tag(ai_reply, db, workspace_id, user_id, caso_id)

    ai_message = {
        "role": "ai",
        "content": encrypt_field(ai_reply),
        "timestamp": datetime.now(timezone.utc),
        "workspace_id": workspace_id,
        "user_id": "maia",
    }
    if caso_id:
        ai_message["caso_id"] = caso_id
    await collection.insert_one(ai_message)

    return ai_reply

async def send_message_stream(
    db: AsyncIOMotorDatabase,
    message: str,
    ai_provider: AIProvider,
    workspace_id: str,
    user_id: str,
    caso_id: Optional[str] = None,
) -> AsyncGenerator[str, None]:
    collection, context, full_prompt, rag_chunks, legal_chunks = await _prepare_chat_context(
        db, message, workspace_id, user_id, caso_id
    )

    from services.saas_service import record_ai_telemetry
    async def usage_cb(in_t, out_t):
        await record_ai_telemetry(db, workspace_id, in_t, out_t, route="chat_stream")

    full_reply = ""
    async for chunk in ai_provider.generate_stream(
        full_prompt, context, rag_context=rag_chunks, legal_context=legal_chunks, usage_callback=usage_cb
    ):
        full_reply += chunk
        yield chunk

    full_reply, success_msg = await _process_maia_save_tag(full_reply, db, workspace_id, user_id, caso_id)
    if success_msg:
        yield success_msg

    ai_message = {
        "role": "ai",
        "content": encrypt_field(full_reply),
        "timestamp": datetime.now(timezone.utc),
        "workspace_id": workspace_id,
        "user_id": "maia",
    }
    if caso_id:
        ai_message["caso_id"] = caso_id
    await collection.insert_one(ai_message)

async def clear_chat_history(
    db: AsyncIOMotorDatabase,
    workspace_id: str,
    caso_id: Optional[str] = None,
) -> int:
    query = {"workspace_id": workspace_id, "caso_id": caso_id} if caso_id else {"workspace_id": workspace_id, "caso_id": {"$exists": False}}
    result = await db[COLLECTION_NAME].delete_many(query)
    return result.deleted_count
